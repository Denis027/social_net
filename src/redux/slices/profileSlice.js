...
export const profileSlice = createSliceWithThunks({
  ...
  reducers: (create) => ({
    ...
    getProfileStatus: create.asyncThunk(
      async (userId) => {
        const response = await profileAPI.getProfileStatus(userId || 30973);
        return response;
      },
      {
        pending: (state) => {
    ...
    editProfileStatus: create.asyncThunk(
      async (profileStatus) => {
        const response = await profileAPI.setProfileStatus(profileStatus);
      ...
      },
      {
        ...
        fulfilled: (state, action) => {
          ...
          if (action.payload.resultCode === 0) {
            state.profileData.profileStatus = state.profileData.newStatusText;
          }
        },
        rejected: (state, action) => {
          state.status = "error";
